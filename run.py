...
if args.dataset == 'h36m':
    ...
    dataset = MpiInf3dhpDataset(dataset_path)
...
model_pos_train = FMFormer(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs,
        num_heads=8, mlp_ratio=2, qkv_bias=True, qk_scale=None, drop_path_rate=0.1, edge_pad=args.edgepad)

model_pos = FMFormer(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs,
        num_heads=8, mlp_ratio=2, qkv_bias=True, qk_scale=None, drop_path_rate=0, edge_pad=args.edgepad)
#################
...
def eval_data_prepare(receptive_field, inputs_2d, inputs_3d):
    ...
    eval_input_2d = torch.empty(out_num, receptive_field+args.edgepad*2, inputs_2d_p.shape[1], inputs_2d_p.shape[2])
    eval_input_3d = torch.empty(out_num, receptive_field, inputs_3d_p.shape[1], inputs_3d_p.shape[2])

    if out_num > 1:
        inputs_2d_p = rearrange(inputs_2d_p, 'b f c -> f c b')
        inputs_2d_p = F.pad(inputs_2d_p, (args.edgepad, args.edgepad), mode='replicate')
        inputs_2d_p = rearrange(inputs_2d_p, 'f c b -> b f c')

        for i in range(out_num-1):
            eval_input_2d[i, :, :, :] = inputs_2d_p[i * receptive_field:(i+1) * receptive_field + args.edgepad*2, :, :]
            eval_input_3d[i, :, :, :] = inputs_3d_p[i * receptive_field:(i+1) * receptive_field, :, :]

        eval_input_2d[-1, :, :, :] = inputs_2d_p[-receptive_field-args.edgepad*2:, :, :]
        eval_input_3d[-1, :, :, :] = inputs_3d_p[-receptive_field:, :, :]

    else:
        pad_right = receptive_field-inputs_2d_p.shape[0]
        inputs_2d_p = rearrange(inputs_2d_p, 'b f c -> f c b')
        inputs_2d_p = F.pad(inputs_2d_p, (args.edgepad, pad_right+args.edgepad), mode='replicate')
        inputs_2d_p = rearrange(inputs_2d_p, 'f c b -> b f c')
        inputs_3d_p = rearrange(inputs_3d_p, 'b f c -> f c b')
        inputs_3d_p = F.pad(inputs_3d_p, (0, pad_right), mode='replicate')
        inputs_3d_p = rearrange(inputs_3d_p, 'f c b -> b f c')
        eval_input_2d[0, :, :, :] = inputs_2d_p[:receptive_field+args.edgepad*2, :, :]
        eval_input_3d[0, :, :, :] = inputs_3d_p[:receptive_field, :, :]
...
if not args.evaluate:
    ...
    train_generator = ChunkedGenerator_Seq(args.batch_size//args.stride, cameras_train, poses_train, poses_train_2d, args.number_of_frames,
                                           ...
                                           joints_right=joints_right, edge_pad=args.edgepad)
    ...
    while epoch < args.epochs:
        ...
        for cameras_train, batch_3d, batch_2d in train_generator.next_epoch():
            ...
            elif args.dataset=='mpi_inf_3dhp':
                w_mpjpe = torch.tensor([1.5, 1, 2.5, 4, 4, 2.5, 4, 4, 1.5, 2.5, 2.5, 1.5, 2.5, 2.5, 1, 1, 1]).cuda()
            elif args.dataset == 'mpi_inf_3dhp_j14':
                w_mpjpe = torch.tensor([1.5, 1, 2.5, 4, 4, 2.5, 4, 4, 1.5, 2.5, 2.5, 1.5, 2.5, 2.5]).cuda()
            ...
            loss_diff = 0.5 * dif_seq + 2.0 * ms_velocity_error(predicted_3d_pos, inputs_3d)
...
if args.render:
    ...
    if args.viz_output is not None:
        ...
        if args.compare:
            ...
            anim_output = {'Reconstruction': ground_truth + np.random.normal(loc=0.0, scale=0.1,
                                                                             size=[ground_truth.shape[0], 17, 3])}
    ...
    def run_evaluation(actions, action_filter=None):
        ...
        print('Protocol #1   (MPJPE) action-wise average:', np.mean(errors_p1), 'mm')
        print('Protocol #2 (P-MPJPE) action-wise average:', np.mean(errors_p2), 'mm')
        print('Protocol #3 (N-MPJPE) action-wise average:', np.mean(errors_p3), 'mm')
        print('Velocity      (MPJVE) action-wise average:', np.mean(errors_vel), 'mm')
