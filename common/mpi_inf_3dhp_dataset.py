...
mpi_inf_3dhp_skeleton = Skeleton(parents=[-1, 16, 1, 2, 3, 1, 5, 6, 14, 8, 9, 14, 11, 12, 15, 1, 0],
...
class MpiInf3dhpDataset(MocapDataset):
    def __init__(self, path, remove_static_joints=False):
        ...
        if remove_static_joints:
            # 14-joint
            self._skeleton.remove_joints([14, 15, 16])

            # Rewire shoulders to the correct parents
            self._skeleton._parents[1] = 0
            self._skeleton._parents[8] = 1
            self._skeleton._parents[11] = 1
