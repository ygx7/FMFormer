...
class ChunkedGenerator_Seq:
    ...
    def __init__(self, batch_size, cameras, poses_3d, poses_2d,
        ...
        for i in range(len(poses_2d)):
            assert poses_3d is None or poses_2d[i].shape[0] == poses_3d[i].shape[0]
            n_chunks = (poses_2d[i].shape[0] + chunk_length - 1) // chunk_length
            offset = (n_chunks * chunk_length - poses_2d[i].shape[0])
            bounds = np.arange(n_chunks+1)*chunk_length
            bounds2 = bounds.copy()
            bounds[-2] -= offset
            bounds2[-1] -= offset
            augment_vector = np.full(len(bounds - 1), False, dtype=bool)
            pairs += zip(np.repeat(i, len(bounds - 1)), bounds[:-1], bounds2[1:], augment_vector)
            if augment:
                pairs += zip(np.repeat(i, len(bounds - 1)), bounds[:-1], bounds2[1:], ~augment_vector)
        ...
        self.batch_2d = np.empty((batch_size, chunk_length+edge_pad*2, poses_2d[0].shape[-2], poses_2d[0].shape[-1]))
        ...
        while enabled:
            ...
            for b_i in range(start_idx, self.num_batches):
                ...
                for i, (seq_i, start_3d, end_3d, flip) in enumerate(chunks):
                    start_2d = start_3d - self.edge_pad  # shift receptive_field / 3
                    end_2d = end_3d + self.edge_pad
